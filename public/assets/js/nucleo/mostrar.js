...
if(varNombreSolicitud){
	$.ajax({
		...
		for(var i=0;i<response.length;i++){
			datatable.row.add([
				response[i].tx_menu,
				response[i].tx_enlace,
				'<span class="pcoded-micon"><i class="'+response[i].tx_icono+'"></i></span>',
				response[i].nu_consecutivo,
				'<button class="btn btn-default" onclick="funcLlenarFormularioUpdate('+response[i].cd_menu+',\'/menus.consultar.reg/\')">Actualizar</button>'+
